...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.label_nc == 0:
            ...
            print('A_tensor min {} A tensor {} max'.format(np.min(np.array(A_tensor)),np.max(np.array(A_tensor))))
        else:
        ...
        if self.opt.isTrain or self.opt.use_encoded_image:
            ...
            print('B_tensor min {} B_tensor {} max'.format(np.min(np.array(B_tensor)),np.max(np.array(B_tensor))))
