...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset_train, start=epoch_iter):
        ...
        save_fake = total_steps % opt.display_freq == display_delta
         ############## Forward Pass ######################
        #generated = (16,3,256,256)
        losses, generated = model(
            Variable(data['af_cy3']),
            Variable(data['af_dapi']),
            Variable(data['inst']),
            Variable(data['image']), 
            Variable(data['feat']), 
            infer=True)
        ...
        ### display output images and compute metrics
        if epoch % 10 == 0:
            print(f'generated [0,:,:,:] shape {generated[0,:,:,:].size()}')
            # imtype = np.uint8 if opt.data_type == '8' else np.uint16
            visuals = OrderedDict([('input_cy3',
                                    util.tensor2label(data['af_cy3'][0],
                                                    opt.label_nc,
                                                    imtype=np.float32)),
                                    ('input_dapi',
                                    util.tensor2label(data['af_dapi'][0],
                                                    opt.label_nc,
                                                    imtype=np.float32)),
                                    ('synthesized_image',
                                    util.tensor2im(generated[0],
                                    imtype=np.float32)),
                                    ('real_image', 
                                    util.tensor2im(data['image'][0],
                                    imtype=np.float32))])
            # Save visuals of of val only        
            ...
            gen_image_gray = color.rgb2gray(visuals['synthesized_image'])
            gt_image_gray = color.rgb2gray(visuals['real_image'])
            channel_score_ssim = ssim(gt_image_gray, gen_image_gray,data_range=gt_image_gray.max()-gt_image_gray.min())
            channel_score_psnr = psnr(gt_image_gray, gen_image_gray,data_range=gt_image_gray.max()-gt_image_gray.min())     
            ssim_scores.append(channel_score_ssim)
            psnr_scores.append(channel_score_psnr)
    ...
    for data in dataset_val:
        _, generated = model(Variable(data['af_cy3']),
                             Variable(data['af_dapi']),
                             Variable(data['inst']),
                             Variable(data['image']),
                             Variable(data['feat']),
        ...
        if epoch % 10 == 0:
            visuals_val = OrderedDict(
                [
                ('input_cy3',util.tensor2label(data['af_cy3'][0],opt.label_nc,imtype=np.float32)),
                ('input_dapi',util.tensor2label(data['af_dapi'][0],opt.label_nc,imtype=np.float32)),
                ('synthesized_image', util.tensor2im(generated[0],imtype=np.float32)),
                ('real_image',util.tensor2im(data['image'][0],imtype=np.float32))
                ]
                )
            ...
            gen_image_gray = color.rgb2gray(visuals_val['synthesized_image'])
            gt_image_gray = color.rgb2gray(visuals_val['real_image'])
