...
for i, data in enumerate(dataset):
    ...
    save_path = os.path.join(
            opt.results_dir, '%s_%s' %
            (opt.phase, opt.which_epoch),
        "16bit_predictions_2",
        )
    
    # Normalize to 8bit
    scale_arr = (visuals['synthesized_image'] - visuals['synthesized_image'].min()) / (visuals['synthesized_image'].max() - visuals['synthesized_image'].min())
    scale_arr = scale_arr * 65535
    scale_arr = scale_arr.astype(np.uint16)
    # Prediction
    end_name = opt.results_dir.split('/')[-1].split('_')[1]
    if end_name == 'cy5':
        final_pred_path = os.path.join(save_path, img_name[:23] + "_4.tiff")
    elif end_name == 'dna':
        final_pred_path = os.path.join(save_path, img_name[:23] + "_1.tiff")
    elif end_name == 'fitc':
        final_pred_path = os.path.join(save_path, img_name[:23] + "_3.tiff")
    imsave(final_pred_path, scale_arr.astype(np.uint16),imagej=True)
    # Input
    # final_input_path = os.path.join(save_path, img_name[:23]+"_input_label.tiff")
    # imsave(final_input_path, visuals['input_label'].astype(np.uint8),imagej=True)
